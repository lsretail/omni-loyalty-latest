...
namespace Presentation.Screens
{
	...
	public class SlideoutNavigationController : UIViewController
	{
		...
		private bool _rightMenuEnabled = true;
		...
		public bool RightMenuEnabled {
			...
			set {
				...
				if (_internalTopNavigation != null && _internalTopNavigation.ViewControllers.Length > 0) {
					//var view = _internalTopNavigation.ViewControllers [0];
					// LSRetail: Outcommented 280514. Let's handle the right menu buttons ourselves.
		...
		public SlideoutNavigationController ()
		{
			...
			SlideHeight = 44f + 20f;
			LayerShadowing = true;
			ShadowOpacity = 0.25f;//0.5f;	// LSRetail: Make the shadow lighter
		...
		public override void ViewWillDisappear (bool animated)
		{
			...
			// LSRetail 280414
			// Outcommented SetNavigationBarHidden. This gives an ugly animation on the navbar when we present viewcontrollers (modal).
			/*if (NavigationController != null)
		...
		{
			// LSRetail: Make the shadow shorter
			//ShowShadow (-5);
			ShowShadow(-1);
		}
			...
			UIView.Animate (SlideSpeed, 0, UIViewAnimationOptions.CurveEaseInOut,
				...
				() =>
				{
					...
					if (MenuViewLeft != null)
					{
						...
						// LSRetail: Added code 180614
						//Utils.UI.HideStatusBarWithAnimation();
					}
				});
		}
		...
		{ 
			// Have to have some title for some reason, otherwise the button is shifted downwards (bug)
			// On the iPhone 5 we can see the first letter of the title to the right of the button (bug)
			// Hack the bugs away by giving the button a space as a title
			/*
			UIBarButtonItem leftBarBtn = new UIBarButtonItem(" ", UIBarButtonItemStyle.Plain, (s, e) => ShowMenuLeft ());
			leftBarBtn.Image = Presentation.Utils.Image.FromFile ("IconSlideMenu.png");

			return leftBarBtn;
			*/

			UIButton btnSlideMenu = new UIButton (UIButtonType.Custom);
			btnSlideMenu.SetImage (Utils.UI.GetColoredImage(Presentation.Utils.Image.FromFile ("IconSlideMenu.png"), UIColor.White), UIControlState.Normal);
			btnSlideMenu.ImageEdgeInsets = new UIEdgeInsets(4, 4, 4, 4);
			btnSlideMenu.Frame = new CGRect (0, 0, 30, 30);
			btnSlideMenu.TouchUpInside += (sender, e) => 
			{
				ShowMenuLeft();
			};

			return new UIBarButtonItem(btnSlideMenu);
		}
		...
		{
			// LSRetail 280514: Function not in use.
			// Let's handle the right barbuttonitems ourselves on a controller-to-controller basis.
			// Why? Because we want to be able to have a list of rightbarbuttonitems which this slideoutnavigationcontroller doesn't support.
			// Also, this only adds the rightbarbuttonitem to the first level in the navigationcontroller's viewcontroller hierarchy.

			/*
			UIButton btnBasket = new UIButton (UIButtonType.Custom);
			btnBasket.SetImage (Presentation.Utils.Image.FromFile ("iconShoppingBasketWhite.png"), UIControlState.Normal);
			btnBasket.Frame = new RectangleF (0, 0, 20, 20);
			btnBasket.TouchUpInside += (sender, e) => 
			{
				ShowMenuRight ();
			};

			UIBarButtonItem rightBarBtn = new UIBarButtonItem (btnBasket);
		...
		public void Hide (bool animate = true)
		{
			...
			Action finished = () => {
				...
				// LSRetail: Added code 180614
				//Utils.UI.ShowStatusBarWithAnimation();
			};
