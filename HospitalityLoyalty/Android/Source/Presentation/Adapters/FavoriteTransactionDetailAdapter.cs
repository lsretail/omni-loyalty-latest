...
{
    public class FavoriteTransactionDetailAdapter : BaseRecyclerAdapter
    {
        ...
        public FavoriteTransactionDetailAdapter(Context context, IItemClickListener listener, int columns)
        {
        ...
        public override async void OnBindViewHolder(RecyclerView.ViewHolder viewHolder, int position)
        {
            ...
            if (type == 0)
            {
                var favoriteTransactionDetailHeaderViewHolder = viewHolder as FavoriteTransactionDetailHeaderViewHolder;

                favoriteTransactionDetailHeaderViewHolder.Title.Text = transaction.Name;
            ...
            { 
                var favoriteItemViewHolder = viewHolder as FavoriteTransactionDetailItemViewHolder;
        ...
        public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType)
        {
            ...
            if (viewType == 0)
            {
                View view = Utils.Utils.ViewUtils.Inflate(LayoutInflater.From(parent.Context), Resource.Layout.FavoriteTransactionDetailHeader, parent);
                ...
                vh = new FavoriteTransactionDetailHeaderViewHolder(view, (itemClickedType, pos) =>
                {
                    ...
                    {
                        listener.ItemClicked(ItemType.Delete, "", "", 0, view);
                    }
                });
            }
            else
            {
                View view = Utils.Utils.ViewUtils.Inflate(LayoutInflater.From(parent.Context), Resource.Layout.FavoriteTransactionDetailCardItem, parent);

                vh = new FavoriteTransactionDetailItemViewHolder(view, async (itemClickedType, pos) =>
                {
                    ...
                    {
                        await favoriteModel.ToggleFavorite(saleLine.Item, false);
                        //NotifyItemChanged(pos);
                    }
                    else if (itemClickedType == ItemClickedType.Item)
                    {
                        listener.ItemClicked(ItemType.Item, saleLine.Id, saleLine.Item.MenuId, (int)menuService.GetItemType(saleLine.Item), view, string.Empty);
                    }
        ...
        private class FavoriteTransactionDetailHeaderViewHolder : RecyclerView.ViewHolder, View.IOnClickListener
        {
            ...
            public FavoriteTransactionDetailHeaderViewHolder(IntPtr javaReference, JniHandleOwnership transfer)
                : base(javaReference, transfer)
            {
            }

            public FavoriteTransactionDetailHeaderViewHolder(View view, Action<ItemClickedType, int> itemClicked)
                : base(view)
            {
                this.itemClicked = itemClicked;

                Title = view.FindViewById<TextView>(Resource.Id.FavoriteTransactionDetailHeaderTitle);
                SubTitle = view.FindViewById<TextView>(Resource.Id.FavoriteTransactionDetailHeaderSubTitle);
                Price = view.FindViewById<TextView>(Resource.Id.FavoriteTransactionDetailHeaderPrice);
                Delete = view.FindViewById<Button>(Resource.Id.FavoriteTransactionDetailHeaderDelete);
                AddToBasket = view.FindViewById<Button>(Resource.Id.FavoriteTransactionDetailHeaderAddToBasket);
            ...
            public void OnClick(View v)
            {
                switch (v.Id)
                {
                    case Resource.Id.FavoriteTransactionDetailHeaderDelete:
                        itemClicked(ItemClickedType.Delete, AdapterPosition);
                        break;

                    case Resource.Id.FavoriteTransactionDetailHeaderAddToBasket:
        ...
        private class FavoriteTransactionDetailItemViewHolder : RecyclerView.ViewHolder, View.IOnClickListener
        {
            private readonly Action<ItemClickedType, int> itemClicked;
            ...
            public FavoriteTransactionDetailItemViewHolder(IntPtr javaReference, JniHandleOwnership transfer)
                : base(javaReference, transfer)
            {
            }

            public FavoriteTransactionDetailItemViewHolder(View view, Action<ItemClickedType, int> itemClicked)
                : base(view)
            {
                this.itemClicked = itemClicked;

                ImageContainer = view.FindViewById<FrameLayout>(Resource.Id.FavoriteTransactionDetailListItemViewItemImageContainer);
                Image = view.FindViewById<ImageView>(Resource.Id.FavoriteTransactionDetailListItemViewItemImage);
                Title = view.FindViewById<TextView>(Resource.Id.FavoriteTransactionDetailListItemViewTitle);
                Qty = view.FindViewById<TextView>(Resource.Id.FavoriteTransactionDetailListItemViewQty);
                SubTitle = view.FindViewById<TextView>(Resource.Id.FavoriteTransactionDetailListItemViewSubtitle);
                Price = view.FindViewById<TextView>(Resource.Id.FavoriteTransactionDetailListItemViewPrice);
                Favorite = view.FindViewById<ImageButton>(Resource.Id.FavoriteTransactionDetailListItemViewFavorite);
                AddToBasket = view.FindViewById<ImageButton>(Resource.Id.FavoriteTransactionDetailListItemViewAddToBasket);
            ...
            public void OnClick(View v)
            {
                switch (v.Id)
                {
                    case Resource.Id.FavoriteTransactionDetailListItemViewFavorite:
                        itemClicked(ItemClickedType.Favorite, AdapterPosition);
                        break;

                    case Resource.Id.FavoriteTransactionDetailListItemViewAddToBasket:
                        itemClicked(ItemClickedType.AddToBasket, AdapterPosition);
                        break;
